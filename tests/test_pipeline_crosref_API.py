...
import pytest
import requests
from pipeline import fetch_crossref, normalize_doi


def _cache_path_for(doi: str) -> str:
    """Return the expected cache/crossref path for a given DOI (mirrors pipeline logic)."""
    normalized = normalize_doi(doi)
    doi_hash = hashlib.sha256(normalized.encode("utf-8")).hexdigest()
    return os.path.join("cache", "crossref", f"{doi_hash}.json")

TEST_DOI = "doi:10.1007/s10671-016-9199-2"
CACHE_DIR = "cache/crossref"


def test_fetch_crossref_cache_behavior():
    # Clean cache before test
    if os.path.exists(CACHE_DIR):
        shutil.rmtree(CACHE_DIR)

    # First call (should hit network and create cache)
    data1 = fetch_crossref(TEST_DOI)
    ...
    data2 = fetch_crossref(TEST_DOI)

    assert data1 == data2


def test_fetch_crossref_retries_and_raises_without_cache(monkeypatch):
    if os.path.exists(CACHE_DIR):
        shutil.rmtree(CACHE_DIR)
    ...
    cache_path = _cache_path_for(test_doi)

    if os.path.exists(cache_path):
        os.remove(cache_path)

    with pytest.raises(requests.exceptions.Timeout):
        fetch_crossref(test_doi)

    assert call_count["n"] == 3
    assert not os.path.exists(cache_path)


def test_fetch_crossref_uses_cache_on_network_failure(monkeypatch):
    ...
    # Use the SHA-256 hash naming that pipeline.py uses
    cache_path = _cache_path_for(test_doi)
